...
import {View, Text, Pressable,TextInput, PermissionsAndroid, Platform, TouchableOpacity,SafeAreaView, FlatList} from 'react-native';
...
import {visitStore,getUserStore, leaveVisitedStore,currentVisitedStore, getAgentStores} from '@Request2/Store';
...
const CustomerStore = props => {
  ...
  const sheetHeight = Platform.OS === "android" ? 480 : 510

const id = props?.id;

  const { status, customers } = useSelector((state) => state.customer);
  const {visitStatus,leaveStatus, currentStoreData, agentStoreErrors, agentStoreStatus,usersStore,leaveErrors, errors} = useSelector(state => state.store);
  ...
  console.log("the eeeeee", id.id)


  useEffect(() => {

    if (search == "") {
      setResult(usersStore?.stores)
    }
  ...
  return (
  <SafeAreaView>
    <BottomSheet  ref={props.bottomSheetStores} height={sheetHeight} radius={20} sheetBackgroundColor="#F2F4F7"  hasDraggableIcon={true} >
        <View style={styles.body5}>
          <View style={styles.headerStyle}>
            <TouchableOpacity style={styles.touchCover} onPress={props.returnBackStores}>
              <Icon name="x" size={22} color="#000" />
            </TouchableOpacity>
            <Text style={styles.headerTitle}>Customer Store(s)</Text>
          </View>
         ...
         <FlatList
            data={!result?.length ? usersStore?.stores : result }
