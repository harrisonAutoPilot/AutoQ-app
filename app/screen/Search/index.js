...
import { searchProducts, addToWishlistProducts, searchProductsByItems } from "@Request/products";
import styles from "./style";
import ListItems from "@Screen/BrowseProducts/ListView";
import ModalView from "@Screen/CartOverlay";
import globalStyle from "@Helper/globalStyles";
import style from "@Screen/Browse/style";
...
const Search = (props) => {
    ...
    const searchCategoryItem = () => {
        // let searched = categories.filter(val => {
        //     if (val.name.toLowerCase().includes(searchCategory.toLowerCase())) {
        //         return val
        //     }
        //     return null
        // });
...
const clear = () => {
    ...
    //     <TouchableOpacity style={[style.listContainer, style.elevation]} onPress={() => showMapCategory(item.name)}>
    //         <View style={style.listContainerImageView}>
    //             <Image source={{ uri: `${URL}${item.product_images[0].url}` }} style={style.image} resizeMode="contain" />
    //         </View>
    //         <View style={style.listContainerTextView}>
    //             <View>
    //                 <Text style={[globalStyle.title, style.title]}>{item.name}</Text>
    //             </View>
    //             <View >
