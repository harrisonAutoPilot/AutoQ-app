...
const GenProducts = (props) => {
    ...
    const {kessingtonStatus, kessingtons,kessingtonItems} = useSelector((state) => state.kessington);

    const {bisStatus, backInStocks,backInStocksItems} = useSelector((state) => state.backInStock);

    const {newProductStatus, newProducts, newProductItems} = useSelector((state) => state.newProduct);

    const {status,errors, deals, addDealStatus,dealsItems, addDeal } = useSelector((state) => state.deal);

    const {popularProducts,popularProductItems} = useSelector((state) => state.popularProduct);
    ...
    return (
        <View style={styles.body}>
            ...
            <FlatList
                ...
                refreshControl={
                        active === "1" ? 
                    <RefreshControl refreshing={refreshing} onRefresh={refreshViewDeals} />
                    : active === "2" ? 
